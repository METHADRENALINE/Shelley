...
class MemoryConnection:
    ...
    def execute(self, sql, params=()):
        ...
        if normalized.startswith("select count(*) as users"):
            ...
            return Cursor({"users": len(rows), "text_points": sum(row["text_points"] for row in rows), "voice_points": sum(row["voice_points"] for row in rows)})
...
def test_config_validation_accepts_structure_and_rejects_runtime_placeholders() -> None:
    from shelley.config import BotConfig, ConfigError

    cfg = BotConfig.model_validate(load_json("config.json"))
...
def test_points_store_award_cooldown_add_remove_reset_and_top() -> None:
    ...
    award = store.award(10, 1, "text", amount=15, now=1000, cooldown=120, name="user", display_name="User", text_channel_id=50, text_message_id=60)
...
def test_voice_points_eligibility_blocks_bots_deaf_mute_and_alone() -> None:
    from shelley.cogs.voice_points import voice_member_is_points_eligible, VoicePointsService
    from shelley.config import BotConfig

    assert voice_member_is_points_eligible(SimpleNamespace(bot=False, voice=SimpleNamespace(deaf=False, self_deaf=False, mute=False, self_mute=False)))
