...
if voice_recv is not None:
    ...
    class _PointsVoiceSink(voice_recv.AudioSink):
    ...
    PointsVoiceSink = _PointsVoiceSink
...
class VoicePointsService:
    ...
    def _voice_client_for_guild(self, guild_id: int) -> Any | None:
        for voice_client in self.bot.voice_clients:
            guild = getattr(voice_client, "guild", None)
            if int(getattr(guild, "id", 0) or 0) == int(guild_id):
                return voice_client
        return None
    ...
    async def _discard_stale_voice_client(self, voice_client: Any, guild_id: int) -> None:
        ...
        try:
            ...
            logger.exception("stale voice client disconnect failed", extra={"guild_id": int(guild_id)})
            cleanup = getattr(voice_client, "cleanup", None)
            if callable(cleanup):
                try:
                    cleanup()
                except Exception:
                    logger.exception("stale voice client cleanup failed", extra={"guild_id": int(guild_id)})

    async def _connect_voice_client(self, target_channel: Any) -> Any:
