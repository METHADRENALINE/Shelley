...
from ..state import clear_starting_status, load_active_starting_statuses, migrate_status_message_ids_for_embed_layout
...
class StatusCog(commands.Cog):
    ...
    async def status_loop(self) -> None:
        ...
        update_seconds = int(cfg.get("update_seconds", 60))
        timeout_seconds = float(cfg.get("timeout_seconds", 3))
            ...
            ServerCfg(
                ...
                components=[
                    ServerComponentCfg(
                        ...
                        tmux_session=component.get("tmux_session"),
                    )
        ...
        status_messages = [
            item
            for item in cfg.get("status_messages", [])
        ...
        for index, message_cfg in enumerate(status_messages):
            ...
            print(f"[{env_name()}] Using status message key={key} id={messages[key].id} in channel={ch.name}")

        while True:
            try:
                snapshots: Dict[str, dict] = {}
                starting_statuses = load_active_starting_statuses(state_path)
                ...
                for s in servers:
                    ...
                    if s.components:
                        ...
                        clear_starting_status(state_path, s.placeholder)
                        snapshots[s.placeholder] = {
                            "status": ":green_circle:",
                ...
                for index, message_cfg in enumerate(status_messages):
                    key = str(message_cfg["key"]).strip().lower()
                    message_type = str(message_cfg.get("type", "embed")).strip().lower()
                    content: Optional[str] = None
                    embeds: List[discord.Embed] = []
                    control_status: Optional[str] = None

                    if message_type == "separator":
                        content = str(message_cfg.get("content", "\u2800\n\u2800"))
                    else:
                        placeholder = str(message_cfg["status_placeholder"])
                        snapshot = snapshots.get(placeholder)
                        if snapshot is None:
                            print(f"[{env_name()}] Missing status snapshot for placeholder={placeholder}")
                            continue

                        renderer = str(message_cfg.get("renderer", "")).strip().lower()
                        template_path = str(message_cfg["template_path"])
                        ...
                        if str(message_cfg.get("control_target", "")).strip().lower() == "bm":
                            control_status = str(snapshot["status"])

                    view = status_message_view(message_cfg, control_status)
                    ...
                    if signature != last_signatures.get(key):
                        try:
                            ...
                            print(f"[{env_name()}] Forbidden editing status message key={key}: {e}")
                        except discord.HTTPException as e:
                            print(f"[{env_name()}] HTTPException editing status message key={key}: {e}")

            except Exception as e:
                print(f"[{env_name()}] status_loop iteration failed: {e!r}")
