...
class PointsCog(commands.Cog):
    ...
    async def _reset(self, interaction: discord.Interaction, kind: Literal["all", "text", "voice"]) -> None:
        guild = await self._require_admin_guild(interaction)
        if guild is None:
            return
        await silent_defer(interaction)
        changed = self.store.reset_points(int(guild.id), kind)
        self.leaderboard.mark_dirty()
        await interaction.followup.send(f"Готово. Обновлено записей: {changed}.", ephemeral=True)

    async def _add(self, interaction: discord.Interaction, kind: Literal["text", "voice"], amount: int, user: discord.Member) -> None:
        guild = await self._require_admin_guild(interaction)
        if guild is None:
            return
        await silent_defer(interaction)
    ...
    async def _remove(self, interaction: discord.Interaction, kind: Literal["text", "voice"], amount: int, user: discord.Member) -> None:
        guild = await self._require_admin_guild(interaction)
        if guild is None:
            return
        await silent_defer(interaction)
